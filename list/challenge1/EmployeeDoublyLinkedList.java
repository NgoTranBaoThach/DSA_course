...
public class EmployeeDoublyLinkedList {
    ...
    public boolean addBefore(Employee newEmployee, Employee existingEmployee) {
        ...
        if (existingEmployee.equals(head.getEmployee())) {
            node.setNext(head);
            head.setPrevious(node);
            head = node;
            size++;
            ...
            while (checkNode.getEmployee() != null) {
                if (checkNode.getEmployee().equals(existingEmployee)) {
                    node.setPrevious(checkNode.getPrevious());
                    node.setNext(checkNode);
                    checkNode.getPrevious().setNext(node);
                    checkNode.setPrevious(node);
                    size++;
                    return true;
                } else {
